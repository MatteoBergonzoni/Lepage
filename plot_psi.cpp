...
// g++ plot_psi.cpp -lgsl -lgslcblas -lm
...
// PLOT EIGENFUNCTION
 ...
 // set global parameters for the Coulomb + short-range potential
 activate_EFT = false;
 activate_dirac = 0;
 c = 0;
 v_depth = -1.5;
 ...
 // find when psi explodes
 double x_i = 0.;
 double x_f = 1000;
 if (nodes_wanted == 1) x_final = psi_boom(E_mean, DISCRETIZATION, x_i, x_f);
 else x_final = psi_boom_min(E_mean, DISCRETIZATION, x_i, x_f);
 // save for the plot
 ofstream ofs2;
 ofs2.open("eigenfunction_file.txt", std::ofstream::out | std::ofstream::trunc); // to clean the file, if it already exists
 ofs2.close(); 
 save_psi (x_i, x_final, DISCRETIZATION, E_mean);
}
